const content = {
  title: 'The Global Pneumococcal Sequencing Project',
  subtitle: {
    content: 'A worldwide genomic survey of the impact of vaccination on the pathogen population'
  },
  mission: {
    title: 'Our Mission',
    content: 'We establish a worldwide genomic surveillance network of *Streptococcus pneumoniae* to provide evidence for pneumococcal disease control by building a decentralised system for local data generation and analysis that will be sustainable into the future.'
  },
  introVideo: {
    title: 'How can whole-genome sequencing be used to make vaccines more effective?',
    id: 'trMW18aKXjo'
  },
  leaders: {
    subtitle: {
      content: 'Project Leaders'
    },
    content: [
      {
        name: 'Stephen Bentley',
        url: 'https://www.sanger.ac.uk/people/directory/bentley-stephen-d',
        institute: 'Wellcome Sanger Institute',
        instituteUrl: 'https://www.sanger.ac.uk/'
      },
      {
        name: 'Lesley McGee',
        url: '',
        institute: 'Centers for Disease Control and Prevention',
        instituteUrl: 'https://www.cdc.gov/'
      }
    ]
  },
  pms: {
    subtitle: {
      content: 'Project Manager'
    },
    content: [
      {
        name: 'Stephanie Lo',
        url: 'https://www.sanger.ac.uk/person/lo-stephanie/',
        institute: 'Wellcome Sanger Institute',
        instituteUrl: 'https://www.sanger.ac.uk/'
      }
    ]
  },
  funder: {
    subtitle: {
      content: 'Funded by'
    },
    content: {
      logo: 'img/gates_foundation_logo.png',
      alt: 'Bill & Melinda Gates Foundation',
      url: 'https://www.gatesfoundation.org/'
    }
  },
  cofunder: {
    subtitle: {
      content: 'Co-funded by'
    },
    content: {
      logo: 'img/wellcome_trust_logo.png',
      alt: 'Wellcome Trust Logo',
      url: 'https://wellcome.org/'
    }
  },
  founders: {
    subtitle: {
      content: 'Founding Partners'
    },
    content: [
      {
        logo: 'img/cdc_logo.png',
        alt: 'CDC Logo',
        url: 'https://www.cdc.gov/'
      },
      {
        logo: 'img/emory_logo.png',
        alt: 'Emory University Logo',
        url: 'https://www.globalhealth.emory.edu/'
      },
      {
        logo: 'img/mlw_logo.png',
        alt: 'Malawi Liverpool Wellcome Programme Logo',
        url: 'https://www.mlw.mw/'
      },
      {
        logo: 'img/mrc_gambia_logo.png',
        alt: 'MRC Unit The Gambia at LSHTM Logo',
        url: 'https://www.mrc.gm'
      },
      {
        logo: 'img/nicd_logo.png',
        alt: 'The National Institute for Communicable Diseases Logo',
        url: 'https://www.nicd.ac.za/'
      },
      {
        logo: 'img/sanger_logo.png',
        alt: 'Wellcome Sanger Institute Logo',
        url: 'https://www.sanger.ac.uk/'
      }
    ]
  }
}

export default content