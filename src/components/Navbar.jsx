...
const Button = ({ title, url, external }) => (
  ...
  ? 
    <a onClick={deactivate} href={url} target="_blank" rel="noreferrer">
      <label tabIndex={0} className="btn btn-primary btn-s rounded-btn">
        <NewWindowIcon className="stroke-current h-5" />
        {title}
      </label>
    </a>
  : 
    <Link onClick={deactivate} to={url}><label tabIndex={0} className="btn btn-primary btn-s rounded-btn">{title}</label></Link>
)
  ...
  <div className="navbar bg-primary text-primary-content h-24 min-h-[6rem]">
    <div className="flex-1">
      <div className="dropdown">
        <label tabIndex={0} className={`btn btn-primary ${breakpointHidden}`}>
      ...
      <Link to="/" className="btn btn-primary h-20 min-h-[5rem]">
